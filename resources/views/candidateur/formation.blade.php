...
@php
    ...
    $jours = today()->diffInDays($fin, false);
...
<a href="{{ route('accueil') }}" class="d-inline-flex align-items-center gap-1 text-decoration-none fw-semibold mb-3">
    <span class="material-symbols-rounded">arrow_back</span> Toutes les formations
</a>
...
<div class="row g-4">
    <div class="col-lg-8">
        <div class="fiche-head mb-4">
            ...
            <div class="fiche-facts">
                ...
                @endif
                @if ($formation->places)
                    <span><span class="material-symbols-rounded">groups</span> {{ $formation->places }} places</span>
                @endif
                @if (config('etablissement.ville'))
                    <span><span class="material-symbols-rounded">location_on</span> {{ config('etablissement.ville') }}</span>
                @endif
                <span><span class="material-symbols-rounded">event</span> Préinscriptions du {{ $debut->format('d/m/Y') }} au {{ $fin->format('d/m/Y') }}</span>
            </div>
        </div>

        <div class="panel mb-4">
            <div class="panel-head"><h3><span class="material-symbols-rounded">info</span> Présentation</h3></div>
            <div class="panel-body fiche-texte">
                @if ($formation->description)
                    {!! nl2br(e($formation->description)) !!}
                @else
                    <span class="text-muted">La présentation détaillée de cette formation sera bientôt disponible.</span>
                @endif
            </div>
        </div>
        ...
        <div class="row g-4">
            @if ($conditions->isNotEmpty())
                <div class="col-md-6">
                    <div class="panel h-100">
                        <div class="panel-head"><h3><span class="material-symbols-rounded">rule</span> Conditions d'accès</h3></div>
            ...
            @if ($modalites->isNotEmpty())
                <div class="col-md-6">
                    <div class="panel h-100">
                        <div class="panel-head"><h3><span class="material-symbols-rounded">how_to_reg</span> Sélection</h3></div>
                        <div class="panel-body">
            ...
            @if ($debouches->isNotEmpty())
                <div class="col-12">
                    <div class="panel">
                        <div class="panel-head"><h3><span class="material-symbols-rounded">work</span> Débouchés</h3></div>
    ...
    <div class="col-lg-4">
            ...
            <div class="panel-body">
                @if ($ouverte)
                    <div class="fiche-compte {{ $jours <= 7 ? 'urgent' : '' }}">
                        <span class="n">{{ max(0, $jours) }}</span>
                        <span>{{ $jours === 0 ? 'dernier jour pour postuler' : 'jour' . ($jours > 1 ? 's' : '') . ' pour postuler' }}</span>
                    </div>
                    <a href="{{ route('candidat.form', ['formation' => $formation->id]) }}" class="btn btn-brand w-100 justify-content-center py-2 mb-2">
                        Postuler à cette formation <span class="material-symbols-rounded">arrow_forward</span>
                    </a>
                    <p class="text-muted small mb-0">Environ 10 minutes. Préparez : pièce d'identité, CV, lettre de demande, photo, scans du bac et des diplômes.</p>
                @else
                    <div class="fiche-compte">
                        <span class="material-symbols-rounded" style="font-size: 32px;">event_upcoming</span>
                        <span>Ouverture des préinscriptions le {{ $debut->translatedFormat('d F Y') }}</span>
                    </div>
                @endif
            </div>
        </div>
        ...
        @if (config('etablissement.email') || config('etablissement.telephone') || config('etablissement.adresse'))
            <div class="panel mb-4">
                <div class="panel-head"><h3><span class="material-symbols-rounded">support_agent</span> Une question ?</h3></div>
                <div class="panel-body small d-grid gap-2">
                    ...
                    @if (config('etablissement.telephone'))<a href="tel:{{ config('etablissement.telephone') }}"><span class="material-symbols-rounded">call</span> {{ config('etablissement.telephone') }}</a>@endif
        ...
        @if ($autres->isNotEmpty())
            <div class="panel">
                <div class="panel-head"><h3><span class="material-symbols-rounded">school</span> Autres formations</h3></div>
                <div class="panel-body d-grid gap-2">
                    @foreach ($autres as $a)
                        <a href="{{ route('formation.public', $a) }}" class="doc-link">
                            <span class="material-symbols-rounded">chevron_right</span>
                            <span><span class="d-block">{{ $a->titre }}</span><span class="person-sub">{{ $a->type_formation }}</span></span>
                        </a>
                    @endforeach
                </div>
            </div>
